...
Release: `hackathon-residence-semantic-2026-09-12-v6`. Classification: **experimental**.
...
```shell
./.venv/Scripts/python.exe -m swisstip.mcp_server.server --release .local/mvp/residence-semantic-2026-09-12-v6/release.json --active-release-id hackathon-residence-semantic-2026-09-12-v6
```

`mcp-requests.json` supplies discovery, 60 resolve examples, 4 negative cases,
3 temporal checks, 4 retrieval-term checks and an evidence request. Select exactly one concept per resolve and supply the
...
`de` and `en` and rank evidence written in the same language by lexical overlap
