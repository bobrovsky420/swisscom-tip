...
from ..concepts import PageNormalizationError
from ..source_plugins import SourceDocument, SourcePlugin, SourceRequest
...
class FedlexPlugin(SourcePlugin):
    ...
    def matches(self, url: str) -> bool:
        ...
        return (parsed.scheme == "https" and parsed.hostname in ("www.fedlex.admin.ch", "fedlex.admin.ch")
                ...
                and re.fullmatch(r"/eli/cc/\d+/[\d_]+/(de|fr|it|en|rm)", parsed.path) is not None)
...
SELECT DISTINCT ?version ?file WHERE {{
  ...
  ?manifestation j:isExemplifiedBy ?file .
    ...
    def normalize(self, path, provenance, **options):
        page = super().normalize(path, provenance, **options)
        if page.title.strip().lower() == "fedlex":
            raise PageNormalizationError("Fedlex application shell is not a legal document")
        return page
