...
The platform-approved catalog is closed and versioned as `tip-language-catalog/v2`. Version 2 replaces the former whole-question and response-language roles with retrieval-term, source and projection roles:

| Language profile | Approved retrieval-term tags | Projection route | Exact source tag |
|---|---|---|---|
| English | `en` | `en` | `en` |
| Swiss Standard German | `de-CH`, `de`, `de-DE` | `de-CH`; reviewed terminology for input variants | `de-CH` |
| French (Switzerland) | `fr-CH` | `fr-CH` | `fr-CH` |
| Italian (Switzerland) | `it-CH` | `it-CH` | `it-CH` |
| Romansh (Switzerland) | `rm-CH` | `rm-CH`, with declared standard/idiom profile | `rm-CH` |
| Swiss German | `gsw`, `gsw-CH` | `de-CH` with reviewed dialect aliases | None |

The catalog enumerates operation-scoped aliases, terminology sets, dialect/idiom profiles and source-language detector mappings. Source declarations require exact source tags; the v2 source-declaration alias set is empty. Bare `fr`, `it` and `rm` are not accepted client retrieval-term tags, even when build-time source detection maps those detector results to a reviewed Swiss profile. This contract does not run a whole-question language detector.

Tags are parsed case-insensitively and emitted with canonical BCP 47 casing, so `DE-de` becomes `de-DE`. Matching is exact after canonicalization. Bare `gsw` is an explicit alias for the `gsw-CH` retrieval-term profile; `de-AT` and other undeclared variants are unsupported. There is no implicit base-language or regional fallback.

A release can enable an evaluated subset of the platform catalog, but cannot introduce tags, aliases, source languages, projection languages, dialects or idioms. Provider capabilities, environment variables and source configuration do not expand coverage. Adding a capability requires a new catalog version and release-gating evaluation. The hackathon P0 target is complete compact projections in `en`, `de-CH`, `fr-CH`, `it-CH` and the declared `rm-CH` form for every included P0 section; a release may claim only its passing evaluated profiles.

A `LanguagePolicy` must be referentially closed: every accepted term alias targets an enabled term profile, every term-to-projection route targets an enabled projection, every source mapping targets an enabled source language, and every declared term/source/projection combination references a passing coverage profile. No cross-product of independently listed languages implies coverage. Unsupported combinations cannot become enabled by model output.
...
- A `SourceDefinition` may declare only exact source tags or source-declaration aliases enumerated for that operation in the platform catalog and enabled by the policy. The v2 source-declaration alias set is empty, so only `en`, `de-CH`, `fr-CH`, `it-CH` and `rm-CH` can be declared before release-level filtering. A malformed tag is a configuration error; a well-formed tag outside the policy is unsupported and blocks the build.
- The builder stores the source-declared tag, detector result, confidence, normalization mapping and effective source language separately. Bare source declarations such as `fr`, `it`, `rm` or `de` are rejected in v2 even though some are valid detector outputs or retrieval-term tags. There is no implicit base-language or regional fallback.
...
For example, the calling LLM interprets `How to get Aufenthaltsbewilligung in Zurich?`, discovers the residence catalog, and clarifies city/canton or applicability facts when needed. TIP receives explicit identifiers and may receive `{"text":"Aufenthaltsbewilligung","language":"de-CH"}` as a term. It does not decide whether the user meant requirements, procedure or another operation. A request for an existing foreign residence status uses the caller-selected foreign-status concept and published context; a Swiss-permit alias must not rewrite that selection.
...
Language fixtures test malformed versus unsupported tags, canonicalization such as `DE-de`, exact tag matching, empty/oversized terms, multiple independently tagged entries, evaluated versus unevaluated term/source combinations, German/Swiss German routes to `de-CH`, accents, Unicode normalization, German compounds and reviewed `ss`/`ß` distinctions. Bare `fr`, `it`, `rm` and undeclared `de-AT` term tags remain unsupported. With no terms, valid identifiers/context can resolve without language detection or a caller-selected pivot. A term language never becomes a source-language filter. Unsupported entries are reported instead of discarded.
