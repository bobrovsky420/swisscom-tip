...
`get_coverage` accepts optional `release_id`, `knowledge_space_id`, `parent_id`, `cursor` and `limit`. With no release it selects the active release for initial discovery and returns its immutable `release_id`; every child request, continuation page and resolution request must pin that identity. Omitted `parent_id` lists the bounded root level within the selected space, or available spaces when no space is selected. A selected parent returns its own details, immediate children and operation/profile metadata. Topic/concept details include the complete applicable context schemas inline, so callers do not need an undocumented schema lookup parameter. The root page (no space and no parent selected) additionally carries `coverage_summary`: the release's sealed scope statement (`scope` and `out_of_scope` per language, from `KnowledgeCatalog.scope`) with an `out_of_scope_response` instruction, its topics, its profile jurisdictions grouped by level, intent and concept count, evidence, label and retrieval-term languages, source snapshot dates and `derived_limits` computed from the profiles and the language policy, so that a caller can decline a question outside the served scope after one call. Pagination has a published default and maximum limit, stable ordering, and opaque cursors bound to the release and selectors. Invalid or mismatched cursors return `INVALID_ARGUMENT`; an unavailable pinned release returns `RELEASE_UNAVAILABLE`.
