...
| Resolved (was minor) | `retrieval_terms` in any language returned `UNSUPPORTED_LANGUAGE` on every real release because no term routes were published, while the pitch leads with five-language retrieval. Resolved 12 September in release v6: same-language routes for `de` and `en` (the cited pages' languages) rank evidence by lexical overlap with the original excerpt; a term in another language is refused by name and a cross-language term is `OUT_OF_COVERAGE`. French has no cited source and no route. The five-language claim stays unproven and the pitch should say so. | [v6 term-route record](../pilots/2026-09-12-retrieval-terms-same-language.md); `mcp-requests.json` `term_checks` |
...
| E12 | **Resolved 12 September** ([record](../pilots/2026-09-12-retrieval-terms-same-language.md)), narrower than proposed. The curated release publishes same-language term routes for `en` and `de` (term language equals source language; the original excerpt is its own complete projection), so a tagged term ranks evidence by lexical overlap instead of erroring. `fr` is not enabled because no cited page is French and the baseline has no projections; cross-language routes wait for the hybrid configuration. The root `coverage_summary` lists the term languages and derives the rule as a limit. Release rebuilt and bundled as v6 (same snapshots and spans). | Removes `UNSUPPORTED_LANGUAGE` from a Swiss server; a small, honest multilingual claim replaces the unproven five-language one. | `build_residence_mvp.py`, `service.py` | Done |
