"""Read public Office document text without launching Office or executing macros.

Legacy Word follows the MS-DOC piece-table text algorithm:
...
def block(kind,text,location,**extra):
    return dict(kind=kind,text=text,heading_path=[],source_locator=location,links=[],**extra)


def result(blocks,kind,**extra):
    return dict(blocks=blocks,representation=kind,html_title=None,main_headings=[],language_declared=None,
                page_kind='office_document',decoding=kind,warnings=['office_layout_not_rendered','macros_not_executed'],**extra)


def extract_openxml(raw):
    blocks=[]
            ...
            for name in sorted(names):
                if re.fullmatch(r'xl/worksheets/sheet\d+\.xml',name):
                    ...
                    for row in root.xpath('//*[local-name()="row"]'):
                        ...
                        rendered=' | '.join(c['value'] if not c['formula'] else c['value']+' [formula: '+c['formula']+']' for c in cells)
                        if rendered.strip(' |'):
...
def legacy_word_pieces(word,table):
    # FIB variable-array lengths locate the shared FibRgFcLcb97 instead of
    # assuming an absolute offset for every Word revision.
    u16=lambda buf,pos:struct.unpack_from('<H',buf,pos)[0]
